    ...
    def play_game(self, p1_type, p2_type):
        ...
        while not g.Check_game_end():
            ...
            if current_player_type == "random":
                ...
                # Ensure fair comparison by using the same N_MAX as DDA1
                self.params.num_mcts_sims = 300 
                az = AlphaZeroMCTS(game=g, net=self.net, params=self.params)
                az.num_moves = turn
                move = az.Run()
            elif current_player_type == "alphadda1":
                # Paper defaults from PeerJ-CS 1123 (Fujita, 2022)
                # A=1000, X0=0.0, num_mean=1, N_MAX=300
                adda = AlphaDDA1MCTS(game=g, net=self.net, params=self.params,
                                     num_mean=1, A=1000, X0=0.0, N_MAX=300)
                adda.num_moves = turn
                move = adda.Run()
            g.Play_action(move)
